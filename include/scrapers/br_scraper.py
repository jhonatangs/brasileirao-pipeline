...
def scrape_brasileirao_future_matches():
    """
    Extrai os jogos FUTUROS (não encerrados) do Brasileirão 2026 da página UOL Esporte.
    Filtra partidas cujo status seja diferente de 'match-ended' e salva em
    data/raw/jogos_futuros_brasileirao_2026.parquet.
    ...
    project_root = os.path.dirname(os.path.dirname(script_dir))
    output_dir = os.path.join(project_root, "data", "raw")
    output_file = os.path.join(output_dir, "jogos_futuros_brasileirao_2026.parquet")
    ...
    # Iterar sobre partidas, filtrando apenas as NÃO encerradas
    parsed_data = []
    for m in re.finditer(r'"football-match-\d+":\s*(\{)', text):
        obj_str = find_braces(text, m.start(1))
        if not obj_str:
        ...
        # Inclui tudo que NÃO é match-ended
        if match_data.get("status") == "match-ended":
            continue
    ...
    if not parsed_data:
        print("Nenhum jogo futuro/em andamento encontrado.")
        return
    ...
    try:
        df.to_parquet(output_file, index=False)
        print(f"Jogos futuros salvos em {output_file} ({len(df)} registros)")
    except Exception as e:
        print(f"Erro ao salvar parquet: {e}")
